                ...
                <tb>
                    <c:if test="${not empty boardList}">
                        <c:forEach var="board" items="${boardList}" varStatus="status">
                            <c:if test="${nameCookie.value == board.regUser && (fn:containsIgnoreCase(board.title, boardSearch.keyword)or fn:containsIgnoreCase(board.no, boardSearch.keyword)or fn:containsIgnoreCase(board.regDate, boardSearch.keyword))}">
                                <tr class="text-center">
                                    <td>
...
<script>
    function changePageSize(select) {
        ...
        var url = window.location.href.split('?')[0] + '?page=1&pageSize=' + pageSize + '&type=${boardSearch.type}&keyword=${boardSearch.keyword}';
        window.location.href = url;
    }

    function fn_paging(page) {
        var url = '?page=' + page + '&pageSize=' + ${pageSize} + '&type=${boardSearch.type}&keyword=${boardSearch.keyword}';
        window.location.href = url;
    }
</script>
